  ...
  return (
    <div className="min-h-screen bg-slate-50">
            ...
            {auth.isAuthenticated && (
              <>
                <NavLink to="/profile" className={({ isActive }) =>
                 ...
                 <NavLink to="/searchbuddy" className={({ isActive }) =>
                  `whitespace-nowrap hover:text-indigo-600 ${isActive ? 'text-indigo-600 font-medium' : 'text-slate-700'}`}>
                    Search Buddy
                  </NavLink>
                ...
                {auth.user?.photoUrl && (
                  <img src={auth.user.photoUrl} alt="me"
      ...
      <main className="px-4 py-6">
        <ErrorBanner />
        <Routes>
          <Route path="/" element={auth.isAuthenticated ? (<Navigate to="/profile" replace />) : ( <Home />)} />
          <Route path="/signup" element={<Signup />} />
          <Route path="/profile" element={auth.isAuthenticated ? <Profile /> : <Home />}/>
          <Route path="/searchbuddy" element={auth.isAuthenticated ? <SearchBuddy/> : <Navigate to="/" replace />} />
