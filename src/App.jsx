...
//   const ready =
//     auth.isAuthenticated && !!auth.user && !!auth.user._id && !!STREAM_API_KEY;

//   useEffect(() => {
//     if (!ready) return; // <-- don’t run until user id exists

//     const client = StreamChat.getInstance(STREAM_API_KEY);
//     let alive = true;
//     const me = {
//       id: String(auth.user._id), // <-- ensure string
//       name: auth.user.fullName || auth.user.email || "User",
//       image:
//         auth.user.photoUrl ||
//         `https://getstream.io/random_svg/?name=${encodeURIComponent(
//           auth.user.fullName || String(auth.user._id)
//         )}`,
//     };
...
//     (async () => {
//       try {
//         // If your backend uses JWT in header:
...
//           method: "POST",
//           headers,
//           // or: credentials: 'include' if you use cookie auth
//           body: JSON.stringify({
//             user_id: me.id,
//             name: me.name,
//             image: me.image,
...
//           throw new Error(
//             `token ${res.status}: ${err.error || err.message || "unknown"}`
//           );
//         }
//         const { token } = await res.json();

//         await client.connectUser(me, token);
//         if (!alive) return;
...
//         update();
//       } catch (e) {
//         console.error("unread-dot init failed", e);
//       }
//     })();
